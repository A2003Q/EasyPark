    ...
    <div class="card admin-card" style="max-width: 600px; margin: 0 auto; background: white; border-radius: 12px; box-shadow: 0 6px 15px rgba(0,0,0,0.1); padding: 30px;">
        <form method="POST" action="{{ route('admin.parkings.update', $parking) }}">
            ...
            <div class="form-group mb-4">
                ...
                <select name="city_id" id="city_id" required
                    ...
                    @foreach($cities as $city)
                        <option value="{{ $city->id }}" {{ $parking->city_id == $city->id ? 'selected' : '' }}>
                            {{ $city->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-group mb-4">
                ...
                <input type="text" id="name" name="name" value="{{ old('name', $parking->name) }}" required
                       style="width: 100%; padding: 10px 12px; border: 1px solid #ddd; border-radius: 8px; font-size: 1rem;">
            </div>

            <div class="form-group mb-4">
                <label for="address" style="display: block; margin-bottom: 8px; font-weight: 600; color: #333;">Address</label>
                <input type="text" id="address" name="address" value="{{ old('address', $parking->address) }}" required
                       style="width: 100%; padding: 10px 12px; border: 1px solid #ddd; border-radius: 8px; font-size: 1rem;">
            </div>

            <div class="form-group mb-4">
                <label for="latitude" style="display: block; margin-bottom: 8px; font-weight: 600; color: #333;">Latitude</label>
                <input type="text" id="latitude" name="latitude" value="{{ old('latitude', $parking->latitude) }}" required
                       style="width: 100%; padding: 10px 12px; border: 1px solid #ddd; border-radius: 8px; font-size: 1rem;">
            </div>

            <div class="form-group mb-4">
                <label for="longitude" style="display: block; margin-bottom: 8px; font-weight: 600; color: #333;">Longitude</label>
                <input type="text" id="longitude" name="longitude" value="{{ old('longitude', $parking->longitude) }}" required
                       style="width: 100%; padding: 10px 12px; border: 1px solid #ddd; border-radius: 8px; font-size: 1rem;">
            </div>

            <div class="form-group mb-4">
                <label for="total_spots" style="display: block; margin-bottom: 8px; font-weight: 600; color: #333;">Total Spots</label>
                <input type="number" id="total_spots" name="total_spots" value="{{ old('total_spots', $parking->total_spots) }}" min="1" required
                       style="width: 100%; padding: 10px 12px; border: 1px solid #ddd; border-radius: 8px; font-size: 1rem;">
            </div>

            <div class="form-group mb-4">
                <label for="available_spots" style="display: block; margin-bottom: 8px; font-weight: 600; color: #333;">Available Spots</label>
                <input type="number" id="available_spots" name="available_spots" value="{{ old('available_spots', $parking->available_spots) }}" min="0" required
                       style="width: 100%; padding: 10px 12px; border: 1px solid #ddd; border-radius: 8px; font-size: 1rem;">
            </div>

            <div class="form-group mb-4">
                <label for="price_per_hour" style="display: block; margin-bottom: 8px; font-weight: 600; color: #333;">Price / Hour (JD)</label>
                <input type="number" id="price_per_hour" name="price_per_hour" value="{{ old('price_per_hour', $parking->price_per_hour) }}" step="0.01" min="0" required
                       style="width: 100%; padding: 10px 12px; border: 1px solid #ddd; border-radius: 8px; font-size: 1rem;">
            </div>
